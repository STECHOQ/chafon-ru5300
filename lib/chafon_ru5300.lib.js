...
class _chafon_ru5300 extends EventEmitter{
	...
	/**
	 *	Connect via TCP and listen to incoming data
	 * */
	_TCPConnect(){
		const self = this;
		...
		self._client.on('error', received => {
			self._emit('error', emitted);
		});

		self._client.on('close', received => {
	...
	async send(cmd, payload = []){
		...
		self._client.write(self._packet(cmd, payload));
